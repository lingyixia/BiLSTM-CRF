...
parser.add_argument("--embeddingSize", type=int, default=128, help='字向量维度')
parser.add_argument("--hiddenSize", type=int, default=512, help='LSTM隐藏层维度')
...
if __name__ == '__main__':
    ...
    os.chdir("/content/drive/ColaboratoryLab/NER/NER_change_trans")
    ...
    params = {'numClasses': len(dataHelper.index2tag), 'vocabSize': len(dataHelper.index2vocab),
              'encodeLayerSize': FLAGS.encode_layer_size, 'eval_tags': eval_tags}
...
test_inputFun = functools.partial(dataHelper.input_fn, os.path.join(FLAGS.data_path, 'test.txt'),
                                  batch_size=FLAGS.batchSize, is_shuffle_and_repeat=False)
